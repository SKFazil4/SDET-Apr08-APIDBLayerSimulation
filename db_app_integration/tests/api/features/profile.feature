Feature: Profile API
  ...
  Scenario: Create profile successfully
    ...
    And I have a profile payload with user_id 1 and bio "Hello, I'm Bob"
    When I send POST request to "/profiles"
    Then the response status code should be 200
    And the response should contain user_id 1 and bio "Hello, I'm Bob"
  ...
  Scenario: Get profile by user ID successfully
    ...
    When I send GET request to "/profiles/userid/1"
    Then the response status code should be 200
    And the response should contain user_id 1 and bio
  ...
  Scenario: Get profile by profile ID successfully
    ...
    When I send GET request to "/profiles/profileid/1"
    Then the response status code should be 200
    And the response should contain profile ID 1

  Scenario: Update profile successfully
    Given a profile exists for user ID 1
    When I send PUT request to "/profiles/id/1" with bio "Updated Bio"
    Then the response status code should be 200
    And the response should contain updated bio "Updated Bio"

  Scenario: Fail to update profile for non-existing user
    When I send PUT request to "/profiles/id/9999"
    Then the response status code should be 404

  Scenario: Delete profile by user ID
    Given a profile exists for user ID 1
    When I send DELETE request to "/profiles/userid/1"
    Then the response status code should be 200

  Scenario: Delete profile by profile ID
    Given a profile exists with profile ID 1
    When I send DELETE request to "/profiles/profileid/1"
    Then the response status code should be 200
