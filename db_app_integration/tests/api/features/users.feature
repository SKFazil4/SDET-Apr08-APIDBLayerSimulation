Feature: User API
  ...
  Scenario: Create a new user successfully
    Given I have a user payload with name "Bob" and email "bob@gmail.com"
    When I send POST request to "/users"
    Then the response status code should be 200
    And the response should contain user name "Bob" and email "bob@gmail.com"
    And the response should contain a valid user ID

  Scenario: Fail to create user with duplicate name
    Given a user already exists with name "Alice" and email "alice@gmail.com"
    When I send POST request to "/users" with name "Alice" and email "alice_new@gmail.com"
  ...
  Scenario: Fail to create user with duplicate email
    Given a user already exists with email "alice@gmail.com"
    When I send POST request to "/users" with name "Alice2" and email "alice@gmail.com"
    Then the response status code should be 409
    And the response message should be "User mail alice@gmail.com already exists"
  ...
  Scenario: Get user by email successfully
    Given a user exists with email "bob@gmail.com"
    When I send GET request to "/users/email/bob@gmail.com"
    Then the response status code should be 200
    And the response should contain email "bob@gmail.com"

  Scenario: Update user successfully
    Given a user exists with ID 1
    When I send PUT request to "/users/id/1" with name "BobUpdated" and email "bob_updated@gmailcom"
    Then the response status code should be 200
    And the response should contain updated name "BobUpdated" and email "bob_updated@gmailcom"

  Scenario: Fail to update non-existing user
    When I send PUT request to "/users/id/9999" with name "BobUpdated" and email "bob_updated@gmailcom"
    Then the response status code should be 404
