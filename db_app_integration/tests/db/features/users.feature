Feature: User Table DB operations
  ...
  Scenario: Create a new user successfully
    Given I have a user payload with name "john" and email "john@gmail.com"
  ...
  Scenario: Fail to insert user with duplicate name
    Given a user already exists with name "mike" and email "mike@gmail.com"
    When I try to insert another user with name "mike" and email "mike_new@gmail.com"
    Then the database should raise a uniqueness constraint error

  Scenario: Fail to insert user with duplicate email
    Given a user already exists with name "kate" and email "kate@gmail.com"
    When I try to insert another user with name "ford" and email "kate@gmail.com"
    Then the database should raise a uniqueness constraint error
  ...
  Scenario: Retrieve user by name
    Given a user exists with name "kate"
    When I query the users table by name "kate"
    Then the returned record should match the user details

  Scenario: Retrieve user by email
    Given a user exists with email "mike@gmail.com"
    When I query the users table by email "mike@gmail.com"
    Then the returned record should match the user details

  Scenario: Update user details
    Given a user exists with ID 5
    When I update the user's name to "john_updated" and email to "john_updated@email.com"
